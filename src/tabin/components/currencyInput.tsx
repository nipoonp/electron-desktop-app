...
const CurrencyInput = (props: {
    ...
    if (props.value !== valueAbsTrunc || !Number.isFinite(props.value) || Number.isNaN(props.value)) {
    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>): void => {
            const { key, keyCode } = e;
            if ((props.value === 0 && !VALID_FIRST.test(key)) || (props.value !== 0 && !VALID_NEXT.test(key) && keyCode !== DELETE_KEY_CODE)) {
                return;
            }
            ...
            if (keyCode !== DELETE_KEY_CODE) {
                const nextValueString: string = props.value === 0 ? key : `${valueString}${key}`;
