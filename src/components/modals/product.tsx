...
export const ProductModal = (props: {
    ...
    product.modifierGroups &&
      product.modifierGroups.items.forEach((modifierGroupLink) => {
        modifierGroupLink.modifierGroup.modifiers &&
          modifierGroupLink.modifierGroup.modifiers.items.map(
            (modifierLink) => {
  ...
  const [orderedModifiers, setOrderedModifiers] =
    useState<IPreSelectedModifiers>(
      editProduct ? editProduct.orderedModifiers : getPreSelectedModifiers()
    );
  const [quantity, setQuantity] = useState(
    editProduct ? editProduct.quantity : 1
  );
  ...
  const [selectedProductModifier, setSelectedProductModifier] =
    useState<ISelectedProductModifier | null>(null);
              ...
              (orderedProductModifierModifierGroup) => {
                orderedProductModifierModifierGroup.modifiers.forEach(
                  (orderedProductModifierModifier) => {
                    ...
                    if (changedQuantity > 0) {
                      price +=
                        orderedProductModifierModifier.price * changedQuantity;
                    }
                  }
                );
              }
  ...
  const onProcessProductModifier = (
    ...
    if (
      selectedProductModifierProduct.modifierGroups &&
      selectedProductModifierProduct.modifierGroups.items.length === 0
    )
      return;
    ...
    if (editSelectionsProductModifierIndex !== undefined) {
      ...
      selectedModifier.productModifiers &&
        selectedModifier.productModifiers[
          editSelectionsProductModifierIndex
                ...
                selectedModifier.productModifier.categories.items.length > 0
                  ? {
                      id: selectedModifier.productModifier.categories.items[0]
                        .category.id,
                      name: selectedModifier.productModifier.categories.items[0]
                        .category.name,
                      kitchenName:
  ...
  const onSubmit = () => {
    ...
    if (editProduct) {
      onUpdateProduct &&
        onUpdateProduct(editProduct.productCartIndex, productToOrder);
    } else {
      ...
      {product.modifierGroups &&
        product.modifierGroups.items.map((mg) => {
          ...
          if (
            ...
            !mg.modifierGroup.availablePlatforms.includes(register.type)
          )
            return;
  ...
  const footer = (
    <>
      ...
      <div className="footer-buttons-container">
        <Button
          className="button large mr-3 cancel-button"
          onClick={onModalClose}
        >
          Cancel
        </Button>
        <Button
          className="button large add-update-order-button"
          onClick={onSubmit}
        >
          {currentSelectedProductModifier
            ? "Save"
    ...
    <>
      <div className="product">
        ...
        <div className="h1 mb-4 name">
          {currentSelectedProductModifier
            ? currentSelectedProductModifier.selectedModifier.name
            : product.name}
        </div>
        {product.description && (
  ...
  const onAddProductModifierProduct = (product: ICartProduct) => {
    ...
    newOrderedModifiers[selectedProductModifier.selectedModifierGroupId][
      lastIndex
    ].productModifiers = [...newProductModifiers, product];
    setOrderedModifiers(newOrderedModifiers);
  };

  const onUpdateProductModifierProduct = (
    index: number,
    productModifier: ICartProduct
  ) => {
    if (
      !selectedProductModifier ||
      selectedProductModifier.editSelectionsProductModifierIndex === undefined
    )
      return; //Check for undefined specifically
    ...
    const productModifierIndex =
      selectedProductModifier.editSelectionsProductModifierIndex;

    const modifier = newOrderedModifiers[
      selectedProductModifier.selectedModifierGroupId
...
export const ModifierGroup = (props: {
  ...
  const [collapsed, setCollapsed] = useState<boolean>(
    modifierGroup.collapsedByDefault ? modifierGroup.collapsedByDefault : false
  );
  const [subModifierGroups, setSubModifierGroups] = useState<string[]>([]);

  const [selectedSubModifierGroup, setSelectedSubModifierGroup] = useState<
    string | null
  >();
    ...
    modifierGroup.modifiers &&
      modifierGroup.modifiers.items.forEach((m) => {
        m.modifier.subModifierGroups &&
          m.modifier.subModifierGroups
            .split(";")
            .forEach((subModifierGroup) => {
              if (!newSubModifierGroups.includes(subModifierGroup))
                newSubModifierGroups.push(subModifierGroup);
            });
      });
  ...
  const checkModifierIsValid = (modifier: IGET_RESTAURANT_MODIFIER) => {
    if (modifier.productModifier) {
      const isSoldOut = isItemSoldOut(
        modifier.productModifier.soldOut,
        modifier.productModifier.soldOutDate
      );
      const isAvailable = isItemAvailable(
        modifier.productModifier.availability
      );
      ...
      const isQuantityAvailable = isModifierQuantityAvailable(
        modifier,
  ...
  const getModifiersSelectedCount = (
    selectedModifiers?: ICartModifier[]
  ): number => {
  ...
  const checkMaxReached = (modifier: IGET_RESTAURANT_MODIFIER) => {
    const isUnlimitedChoice =
      modifierGroup.choiceMin === 0 && modifierGroup.choiceMax === 0;
    const isSingleChoice =
      modifierGroup.choiceMin === 1 && modifierGroup.choiceMax === 1;
    const isModifierSelected = selectedModifiers.some(
      (selectedModifier) => selectedModifier.id === modifier.id
    );
          ...
          {subModifierGroups.length > 0 && (
            <div className="modifier-sub-modifier-group-wrapper mt-4 mb-4">
              {subModifierGroups.map((subModifierGroup) => (
                <div
                  className={`modifier-sub-modifier-group h3 ${
                    selectedSubModifierGroup === subModifierGroup
                      ? "selected"
                      : ""
                  }`}
              ...
              <div
                className={`modifier-sub-modifier-group background-grey h3 ${
                  selectedSubModifierGroup === null ? "selected" : ""
                }`}
...
const Modifier = (props: {
      ...
      return getProductQuantityAvailable(
        {
          ...
          totalQuantityAvailable:
            modifier.productModifier.totalQuantityAvailable,
        },
        cartProductQuantitiesById
      );
    } else {
  ...
  const modifierQuantityAvailable =
    getModifierOrProductModifierQuantityAvailable();
  ...
  const _onChangeModifierQuantity = (
    quantity: number,
    isIncremented: boolean
  ) => {
    if (quantity === 0) {
  ...
  const modifierChildren = (
    <>
      <div className="modifier-item">
            ...
            <div>
              ...
              {modifierQuantityAvailable && modifierQuantityAvailable <= 5 ? (
                <span className="quantity-remaining ml-2">
                  {getQuantityRemainingText(modifierQuantityAvailable)}
                </span>
              ) : (
                <></>
              )}
        ...
        {modifier.productModifier &&
          modifier.productModifier.modifierGroups &&
          modifier.productModifier.modifierGroups.items.length > 0 && (
            <FiChevronRight
              className="product-modifier-chevron-right"
              size={24}
            />
          )}
      </div>
  ...
  const getModifierStepperMax = () => {
    if (modifierQuantityAvailable) {
      let maxSelectable = Math.min(
        choiceDuplicate - modifiersSelectedCount,
  ...
  const collapsedStepper = (
    <div
      className="modifier-item-wrapper collapsed-stepper-container pt-2 pb-2"
      onClick={_onDisplayModifierStepper}
    >
      <div
  ...
  const _radio = (
    <Radio
      className="modifier-item-wrapper pt-2 pb-2"
      selected={checked}
      onSelect={_onSelectRadioModifier}
      disabled={disabled}
    >
      {modifierChildren}
    </Radio>
