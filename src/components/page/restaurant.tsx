...
const Restaurant = () => {
  ...
  const {
    setRestaurant,
    menuCategories: restaurantCategories,
    menuProducts: restaurantProducts,
  } = useRestaurant();
  ...
  const { getProductsBySKUCodeByRestaurant } =
  ...
  useEffect(() => {
    const ticker = setInterval(() => {
      ...
      if (
        userOnPageDuration.current % startShakeAfterSeconds ==
        shakeButtonDurationSeconds
      ) {
    ...
    if (restaurant) {
      ...
      if (selectedCategoryId) {
        ...
        const selectedCategoryItem =
          restaurantCategories[register.defaultCategoryView];
  ...
  useEffect(() => {
    ...
    restaurant.categories.items.forEach((c) => {
      if (c.availablePlatforms && !c.availablePlatforms.includes(register.type))
        return;

      c.products &&
        c.products.items.forEach((p) => {
          if (
            p.product.availablePlatforms &&
            !p.product.availablePlatforms.includes(register.type)
          )
            return;
  ...
  const onClickCart = () => {
    if (
      ...
      if (
        register.availableOrderTypes[0] === EOrderType.DINEIN &&
  ...
  const menuCategories = (
    <>
      {restaurant.categories.items.map((c, index) => {
        if (
          c.availablePlatforms &&
          !c.availablePlatforms.includes(register.type)
        )
          return;
    ...
    <>
      ...
      >
        ...
        >
          <CachedImage
            className="icon"
            url={`${getPublicCloudFrontDomainName()}/images/search-icon.png`}
            alt="search-icon"
          />
  ...
  const onKeyDownSearchProductSKUCode = async (e) => {
    if (e.key === "Enter") {
      ...
      const products: IGET_RESTAURANT_PRODUCT[] =
        res.data.getProductsBySKUCodeByRestaurant.items;
      ...
      if (products.length > 0) {
        ...
        if (
          skuProduct.modifierGroups &&
          skuProduct.modifierGroups.items.length > 0
        ) {
  ...
  const menuMostPopularCategory = (
    <div
      className={`category most-popular-category background-grey ${
    ...
    >
      <CachedImage
        className="icon"
        url={`${getPublicCloudFrontDomainName()}/images/most-popular.png`}
        alt="most-popular-icon"
      />
  ...
  const menuMostPopularProducts = (
    <div>
      {!selectedCategory && (
        <>
          <div className="product-category-name h1 text-center mb-6">
            Most Popular
          </div>
        ...
        restaurant.categories.items.map((c) => {
          ...
          return (
            <>
              ...
              {c.description && (
                <div className="product-category-description text-bold text-center mb-6 h3">
                  {c.description}
                </div>
              )}
              {subCategories.length > 0 && (
                <div className="product-sub-category-wrapper mb-6">
                  {subCategories.map((subCategory) => (
                    <div
                      className={`product-sub-category h3 ${
                  ...
                  <div
                    className={`product-sub-category background-grey h3 ${
                      selectedSubCategory === null ? "selected" : ""
                    }`}
