...
export const Checkout = () => {
  ...
  const { getThirdPartyOrderResponse } =
    useGetThirdPartyOrderResponseLazyQuery();

  // state
  const [selectedCategoryForProductModal, setSelectedCategoryForProductModal] =
    useState<IGET_RESTAURANT_CATEGORY | null>(null);
  const [selectedProductForProductModal, setSelectedProductForProductModal] =
  ...
  const [paymentModalState, setPaymentModalState] =
    useState<EPaymentModalState>(EPaymentModalState.None);

  const [eftposTransactionProcessMessage, setEftposTransactionProcessMessage] =
    useState<string | null>(null);
  const [eftposTransactionOutcome, setEftposTransactionOutcome] =
    useState<IEftposTransactionOutcome | null>(null);
  const [cashTransactionChangeAmount, setCashTransactionChangeAmount] =
    useState<number | null>(null);

  const createdOrder = useRef<IGET_RESTAURANT_ORDER_FRAGMENT | undefined>();

  const [createOrderError, setCreateOrderError] = useState<string | null>(null);
  const [paymentOutcomeOrderNumber, setPaymentOutcomeOrderNumber] = useState<
    string | null
  >(null);
  const [
    paymentOutcomeApprovedRedirectTimeLeft,
    setPaymentOutcomeApprovedRedirectTimeLeft,
  ] = useState(restaurant?.delayBetweenOrdersInSeconds || 10);
  let transactionCompleteRedirectTime =
  ...
  const [showOrderThresholdMessageModal, setShowOrderThresholdMessageModal] =
    useState(false);

  const transactionCompleteTimeoutIntervalId = useRef<
  ...
  const onSelectUpSellCrossSellCategory = (
    category: IGET_RESTAURANT_CATEGORY
  ) => {
    navigate(`${restaurantPath}/${restaurant.id}/${category.id}`);
  };

  const onSelectUpSellCrossSellProduct = (
    category: IGET_RESTAURANT_CATEGORY,
  ...
  const onClickOrderButton = async () => {
    if (
      restaurant.orderThresholds?.enable &&
      restaurant.orderThresholdMessage &&
      !isShownOrderThresholdMessageModal
    ) {
    ...
    if (register && register.requestCustomerInformation) {
      ...
      if (
        register.requestCustomerInformation.firstName &&
        (!customerInformation || !customerInformation.firstName)
      )
        invalid = true;
      if (
        register.requestCustomerInformation.email &&
        (!customerInformation || !customerInformation.email)
      )
        invalid = true;
      if (
        register.requestCustomerInformation.phoneNumber &&
        (!customerInformation || !customerInformation.phoneNumber)
      )
        invalid = true;
      if (
        register.requestCustomerInformation.signature &&
        (!customerInformation || !customerInformation.signatureBase64)
      )
        invalid = true;
    ...
    if (isPOS) {
      ...
      if (
        (paymentMethod === null && register.enableEftposPayments) ||
        paymentMethod === EPaymentMethod.EFTPOS
      ) {
        await onConfirmTotalOrRetryEftposTransaction(subTotal);
      } else if (
        (paymentMethod === null && register.enableCashPayments) ||
        paymentMethod === EPaymentMethod.CASH
      ) {
        await onConfirmCashTransaction(subTotal);
      } else if (
        (paymentMethod === null && register.enablePayLater) ||
        paymentMethod === EPaymentMethod.LATER
      ) {
  ...
  const beginTransactionCompleteTimeout = () => {
    ...
    transactionCompleteTimeoutIntervalId.current = setInterval(() => {
      setPaymentOutcomeApprovedRedirectTimeLeft(
        (prevPaymentOutcomeApprovedRedirectTimeLeft) =>
          prevPaymentOutcomeApprovedRedirectTimeLeft - 1
      );
      timeLeft = timeLeft - 1;

      if (timeLeft == 0) {
        //@ts-ignore
        transactionCompleteTimeoutIntervalId.current &&
          clearInterval(transactionCompleteTimeoutIntervalId.current);
    ...
    if (printer.printerType === EReceiptPrinterPrinterType.LABEL) {
      ...
      await printReceipt({
        ...
        restaurant: {
          ...
          address: `${restaurant.address.aptSuite || ""} ${
        ...
        displayPaymentRequiredMessage:
          ...
          (!isPOS &&
            ...
            order.paymentAmounts.cash === order.subTotal)
            ? true
            : false,
        type: order.type,
        number: order.number,
  ...
  const printReceipts = (order: IGET_RESTAURANT_ORDER_FRAGMENT) => {
    register.printers &&
      register.printers.items.forEach((printer) => {
        if (
          printer.customerPrinter === true &&
          register.askToPrintCustomerReceipt === true
        )
          return;
  ...
  const onPrintParkedOrderReceipts = (
    order: IGET_RESTAURANT_ORDER_FRAGMENT
  ) => {
    register.printers &&
      register.printers.items.forEach((printer) => {
  ...
  const onSubmitOrder = async (
    ...
    let orderNumber =
      parkedOrderId && parkedOrderNumber
        ? parkedOrderNumber
        : getOrderNumber(register.orderNumberSuffix, register.orderNumberStart);
    ...
    try {
      ...
      if (customerInformation && customerInformation.signatureBase64) {
        ...
        const uploadedObject: any = await Storage.put(
          `${filename}.${fileExtension}`,
          signatureFile,
          {
            level: "protected",
      ...
      if (
        register.printers &&
        register.printers.items.length > 0 &&
        !parkedOrderId
      ) {
      ...
      if (
        ...
        setPaymentModalState(
          EPaymentModalState.ThirdPartyIntegrationAwaitingResponse
        );
  ...
  const pollForThirdPartyResponse = (orderId) => {
    ...
    var checkCondition = async (resolve: any, reject: any) => {
      try {
        ...
        const thirdPartyOrderResponse: IGET_THIRD_PARTY_ORDER_RESPONSE =
          thirdPartyOrderResponseRes.data.getOrder;

        if (thirdPartyOrderResponse.thirdPartyIntegrationResult) {
          if (
            thirdPartyOrderResponse.thirdPartyIntegrationResult.isSuccess ===
            true
          ) {
            resolve();
          } else {
            reject(
              thirdPartyOrderResponse.thirdPartyIntegrationResult.errorMessage
            );
          }
          ...
          reject(
            "Third Party Result Polling timed out. Please contact support staff."
          );
          return;
        }
      } catch (error) {
  ...
  const createOrder = async (
    ...
    if (!restaurant) {
      await logError(
        "Invalid restaurant",
        JSON.stringify({ restaurant: restaurant })
      );
  ...
  const retryCreateOrder = async (variables) => {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        const result = await axios.post(
          `https://36p0xwo1cl.execute-api.ap-southeast-2.amazonaws.com/prod`,
          variables
        );
    ...
    await logError(
      `Maximum retry attempts reached. Unable to create order`,
      variables
    );
    ...
    try {
      ...
      if (register.eftposProvider == EEftposProvider.SMARTPAY) {
        ...
        const delayed = (outcome: IEftposTransactionOutcome) => {
          if (!delayedShown) {
            delayedShown = true;
            setEftposTransactionProcessMessage(
              "This transaction is delayed. Please wait..."
            );
          }
        };

        const pollingUrl = await smartpayCreateTransaction(
          amount,
          "Card.Purchase"
        );
        ...
        const setEftposMessage = (message: string | null) =>
          setEftposTransactionProcessMessage(message);
  ...
  const onConfirmTotalOrRetryEftposTransaction = async (amount: number) => {
    ...
    if (outcome.transactionOutcome === EEftposTransactionOutcome.Success) {
      try {
        ...
        const newTotalPaymentAmounts =
          newEftposPaymentAmounts + paymentAmounts.cash;
        ...
        const newPayments: ICartPayment[] = [
          ...payments,
          { type: register.eftposProvider, amount: amount },
        ];
  ...
  const calculateCashChangeAmount = (
  ...
  const onConfirmCashTransaction = async (amount: number) => {
    try {
      ...
      const newPaymentAmounts: ICartPaymentAmounts = {
        ...paymentAmounts,
        cash:
          newTotalPaymentAmounts >= subTotal
            ? subTotal - nonCashPayments
            : newCashPaymentAmounts, //Cannot pay more than subTotal amount
      };
      const newPayments: ICartPayment[] = [
        ...payments,
      ...
      //If paid for everything
      if (newTotalPaymentAmounts >= subTotal) {
        const changeAmount = calculateCashChangeAmount(
          newTotalPaymentAmounts,
          subTotal
        );
  ...
  const onConfirmUberEatsTransaction = async (amount: number) => {
    try {
      ...
      const newTotalPaymentAmounts =
        nonUberEatsPayments + newUberEatsPaymentAmounts;

      const newPaymentAmounts: ICartPaymentAmounts = {
        ...paymentAmounts,
        uberEats:
          newTotalPaymentAmounts >= subTotal
            ? subTotal - nonUberEatsPayments
            : newUberEatsPaymentAmounts, //Cannot pay more than subTotal amount
      };
      const newPayments: ICartPayment[] = [
        ...payments,
        { type: "UBEREATS", amount: amount },
      ];
  ...
  const onConfirmMenulogTransaction = async (amount: number) => {
    try {
      ...
      const newTotalPaymentAmounts =
        nonMenulogPayments + newMenulogPaymentAmounts;

      const newPaymentAmounts: ICartPaymentAmounts = {
        ...paymentAmounts,
        menulog:
          newTotalPaymentAmounts >= subTotal
            ? subTotal - nonMenulogPayments
            : newMenulogPaymentAmounts, //Cannot pay more than subTotal amount
      };
      const newPayments: ICartPayment[] = [
        ...payments,
        { type: "MENULOG", amount: amount },
      ];
  ...
  const upSellCategoryModal = () => {
    if (
      ...
      const upSellCrossSellCategories =
        restaurant.upSellCrossSell.customCategories.items;

      menuCategories.forEach((category) => {
        if (
          category.availablePlatforms &&
          !category.availablePlatforms.includes(register.type)
        )
          return;
  ...
  const upSellProductModal = () => {
    if (
      ...
      const menuCategories = restaurant.categories.items;
      const upSellCrossSellProducts =
        restaurant.upSellCrossSell.customProducts.items;
  ...
  const editProductModal = () => {
    ...
    restaurant.categories.items.forEach((c) => {
      if (
        productToEdit.product.category &&
        productToEdit.product.category.id === c.id
      ) {
        category = c;
      }
    ...
    return (
      <>
        {showPromotionCodeModal && (
          <PromotionCodeModal
            isOpen={showPromotionCodeModal}
            onClose={onClosePromotionCodeModal}
          />
        )}
      </>
    );
        ...
        {showPaymentModal && (
          <PaymentModal
            ...
            onPrintCustomerReceipt={() =>
              ...
              onPrintCustomerReceipt(createdOrder.current)
            }
            onPrintParkedOrderReceipts={() =>
              createdOrder.current &&
              onPrintParkedOrderReceipts(createdOrder.current)
            }
  ...
  const restaurantCustomerInformation = (
    <div className="checkout-customer-details">
      <div className="h3">
        Customer Details:{" "}
        {`${customerInformation?.firstName} ${customerInformation?.email} ${customerInformation?.phoneNumber}`}
      </div>
    ...
    <div>
      ...
      {promotion ? (
        <div className="h3 text-center mb-2">
          ...
          }: -$${convertCentsToDollars(promotion.discountedAmount)}`}{" "}
          {userAppliedPromotionCode && (
            <Link onClick={removeUserAppliedPromotion}> (Remove) </Link>
          )}
        </div>
      ) : (
        <></>
      ...
      {surcharge ? (
        <div className="h3 text-center mb-2">
          Surcharge: ${convertCentsToDollars(surcharge)}
        </div>
      ) : (
        <></>
      )}
      {paidSoFar > 0 ? (
        <div className="h3 text-center mb-2">
          Paid So Far: ${convertCentsToDollars(paidSoFar)}
        </div>
      ) : (
        <></>
      )}
      ...
      <div className={`${isPOS ? "mb-0" : "mb-4"}`}>
        <div className="checkout-buttons-container">
          {!isPOS && (
            <Button
              onClick={onOrderMore}
              className="button large mr-3 order-more-button"
            >
              Order More
            </Button>
          )}
          <Button
            ...
            className="button large complete-order-button"
          >
            Complete Order
          </Button>
        </div>
