...
const ElectronProvider = (props: { children: React.ReactNode }) => {
  ...
  const NoInternetConnection = () => {
    return <div style={{ padding: "64px 48px", textAlign: "center" }}><Title1Font>Lost connection with the internet. Please check for any issues...</Title1Font></div>
  }

  const UpdaterMessage = () => {
    return <div style={{ padding: "64px 48px", textAlign: "center" }}><Title1Font>{electronUpdaterMessage}</Title1Font></div>
  }
...
const useElectron = () => {
  ...
  if (context === undefined) {
    throw new Error(
      `useElectron must be used within a ElectronProvider`
    );
  }
  return context;
};
